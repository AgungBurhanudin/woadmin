...
<main class="main">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">
            <a href="<?= base_url() ?>Setting/Panitia">Panitia</a>
        </li>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <i class="fa fa-align-justify"></i> Data Field Paket Panitia <?= $panitia_tipe->nama_panitia ?>
                        </div>
                        <div class="card-body">
                            <table class="table">
                                <tr>
                                    <td style="width:5%">
                                        <input type="hidden" name="id_panitia" id="id_panitia" value="<?= $panitia_tipe->id ?>">
...
<script>
    ...
    function saveField() {
        ...
        if (valid) {
            hideAlert();
            $.ajax({
                url: "<?= base_url() ?>Setting/Panitia/saveField",
                type: "POST",
                data: "id=" + id + "&id_panitia=" + id_panitia + "&nama_label=" + label + "&nama_field=" + nama_field + "&jenis_field=" + jenis_field + "&deskripsi_field=" + deskripsi_field,
                dataType: "JSON",
                success: function (data) {
                    if (data.resp_code == 200) {
                        showAlert('success', "Berhasil menambah/mengedit field");
                        $("#content_panitia").load(location.href + " #content_panitia");
                        $("#id").val('');
                        $("#nama_field").val('');
                        $("#label").val('');
                        $("#jenis_field").val('');
...
//                        $("#table_body").append(table);
    ...
    function editField(id) {
        $.ajax({
            url: "<?= base_url() ?>Setting/Panitia/getField",
            type: "GET",
            data: "id=" + id,
            dataType: "JSON",
            success: function (data) {
    ...
    function deleteField(id, e) {
        confirmModal('Delete Field', 'Apakah anda yakin akan menghapus field ini', '<?= base_url() ?>Setting/Panitia/deleteField?id=' + id, 'content_panitia');
//        $(e).parent().parent().remove();
    ...
    function saveUrutan(id) {
        ...
        $.ajax({
            url: "<?= base_url() ?>Setting/Panitia/saveUrutan",
            type: "POST",
