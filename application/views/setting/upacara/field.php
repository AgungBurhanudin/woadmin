...
<main class="main">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">
            <a href="<?= base_url() ?>Setting/Upacara/edit?id=<?= $upacara_tipe->id_upacara ?>">Upacara Detail</a>
        </li>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <i class="fa fa-align-justify"></i> Data Field Paket Upacara <?= $upacara_tipe->nama_upacara ?>
                        </div>
                        <div class="card-body">
                            <table class="table">
                                <tr>
                                    <td style="width:5%">
                                        <input type="hidden" name="id_upacara" id="id_upacara" value="<?= $upacara_tipe->id ?>">
...
<script>
    ...
    function saveField() {
        ...
        if (valid) {
            hideAlert();
            $.ajax({
                url: "<?= base_url() ?>Setting/Upacara/saveField",
                type: "POST",
                data: "id=" + id + "&id_upacara=" + id_upacara + "&nama_label=" + label + "&nama_field=" + nama_field + "&jenis_field=" + jenis_field + "&deskripsi_field=" + deskripsi_field,
                dataType: "JSON",
                success: function (data) {
                    if (data.resp_code == 200) {
                        showAlert('success', "Berhasil menambah/mengedit field");
                        $("#content_upacara").load(location.href + " #content_upacara");
                        $("#id").val('');
                        $("#nama_field").val('');
                        $("#label").val('');
                        $("#jenis_field").val('');
...
//                        $("#table_body").append(table);
    ...
    function editField(id) {
        $.ajax({
            url: "<?= base_url() ?>Setting/Upacara/getField",
            type: "GET",
            data: "id=" + id,
            dataType: "JSON",
            success: function (data) {
    ...
    function deleteField(id, e) {
        confirmModal('Delete Field', 'Apakah anda yakin akan menghapus field ini', '<?= base_url() ?>Setting/Upacara/deleteField?id=' + id, 'content_panitia');
//        $(e).parent().parent().remove();
    ...
    function saveUrutan(id) {
        ...
        $.ajax({
            url: "<?= base_url() ?>Setting/Upacara/saveUrutan",
            type: "POST",
